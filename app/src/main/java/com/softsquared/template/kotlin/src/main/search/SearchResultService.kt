...
class SearchResultService (val searchResultFragmentInterface: SearchResultFragmentInterface){
    fun tryGetPoints(keyword:String?=null, order:String?=null, pageId:String){
        val service = ApplicationClass.sRetrofit.create(SearchResultRetrofitInterface::class.java)
        service.getReviews(keyword, order, pageId).enqueue(object : Callback<StarPointResponse> {
            override fun onResponse(call: Call<StarPointResponse>, response: Response<StarPointResponse>) {
                searchResultFragmentInterface.onGetUserSuccess(response.body() as StarPointResponse)
            }

            override fun onFailure(call: Call<StarPointResponse>, t: Throwable) {
                searchResultFragmentInterface.onGetUserFailure(t.message ?: "통신 오류")
            }
        })
    }
